...
public class MoveHard : MonoBehaviour {
    ...
    void Update()
    {
        ...
        if(flagPos != "stop" && text.GetComponent<boxWinConHard>().winner == false)
        {
    ...
    public bool colisao(Vector3 pos)
    {
        ...
        if (Mathf.Approximately(newPos.x, box10.transform.position.x) && Mathf.Approximately(newPos.y, box10.transform.position.y))
            return false;
        if (Mathf.Approximately(newPos.x, box11.transform.position.x) && Mathf.Approximately(newPos.y, box11.transform.position.y))
            return false;
        if (Mathf.Approximately(newPos.x, box12.transform.position.x) && Mathf.Approximately(newPos.y, box12.transform.position.y))
            return false;
        if (Mathf.Approximately(newPos.x, box13.transform.position.x) && Mathf.Approximately(newPos.y, box13.transform.position.y))
            return false;
        if (Mathf.Approximately(newPos.x, box14.transform.position.x) && Mathf.Approximately(newPos.y, box14.transform.position.y))
            return false;
        if (Mathf.Approximately(newPos.x, box15.transform.position.x) && Mathf.Approximately(newPos.y, box15.transform.position.y))
            return false;
        ...
        //out of bounds
        if (pos.x > 2.3 || pos.x < -2.3 || pos.y > 1.5 || pos.y < -1.5)
            return false;
